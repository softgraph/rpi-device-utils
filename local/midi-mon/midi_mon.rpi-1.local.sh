...
while true ; do
	./aconnect_list.sh | tee "$F_MON" | awk '/^t / { sub("^t ", ""); print }' - > "$F_CON"
	if [ ! -s $F_CON ] ; then
		P_SRC=`awk '/P [0-9]+:[0-9]+ '\'"$S_AUTO_CONNECT_SRC"\''/ { print $2 }' $F_MON`
		P_DST=`awk '/P [0-9]+:[0-9]+ '\'"$S_AUTO_CONNECT_DST"\''/ { print $2 }' $F_MON`
